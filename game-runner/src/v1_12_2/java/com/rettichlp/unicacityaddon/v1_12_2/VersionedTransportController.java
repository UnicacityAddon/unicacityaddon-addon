        ...
        if (guiScreen instanceof GuiHopper guiHopper && BusCommand.active) {
            ...
            if (container.windowId != BusCommand.lastWindowId && container instanceof ContainerHopper) {
                ...
                Map<Bus, Slot> busSlotMap = container.inventorySlots.stream()
                        ...
                        .filter(slot -> Bus.getBus(unicacityAddon.services().util().text().stripColor(slot.getStack().getDisplayName())) != null)
                        .collect(Collectors.toMap(slot -> Bus.getBus(unicacityAddon.services().util().text().stripColor(slot.getStack().getDisplayName())), slot -> slot));
