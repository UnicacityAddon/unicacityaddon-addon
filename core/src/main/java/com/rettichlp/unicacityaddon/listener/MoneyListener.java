...
public class MoneyListener {
    ...
    public void onChatReceive(ChatReceiveEvent e) {
        ...
        if (jobSalaryMatcher.find()) {
            this.unicacityAddon.services().file().data().addJobBalance(Integer.parseInt(jobSalaryMatcher.group(1)));
            return;
        }
        ...
        if (jobExperienceMatcher.find()) {
            ...
            if (jobExperienceMatcher.group(3) != null) {
                ...
                this.unicacityAddon.services().file().data().addJobExperience(experience * multiplier);
                return;
            }

            this.unicacityAddon.services().file().data().addJobExperience(experience);
        }
        ...
        if (bankPayDayMatcher.find()) {
            ...
            this.unicacityAddon.services().file().data().setBankBalance(Integer.parseInt(bankPayDayMatcher.group(1)));
            this.unicacityAddon.services().file().data().setJobBalance(0);
            this.unicacityAddon.services().file().data().setJobExperience(0);
            this.unicacityAddon.services().file().data().setPayDayTime(0);
            return;
        }
        ...
        if (bankNewBalanceMatcher.find()) {
            ...
            this.unicacityAddon.services().file().data().setBankBalance(Integer.parseInt(bankNewBalanceMatcher.group(1)));
            return;
        }

        Matcher bankTransferToMatcher = PatternHandler.BANK_TRANSFER_TO_PATTERN.matcher(msg);
        if (bankTransferToMatcher.find()) {
            this.unicacityAddon.services().file().data().removeBankBalance(Integer.parseInt(bankTransferToMatcher.group(2)));
            return;
        }

        Matcher bankTransferGetMatcher = PatternHandler.BANK_TRANSFER_GET_PATTERN.matcher(msg);
        if (bankTransferGetMatcher.find()) {
            this.unicacityAddon.services().file().data().addBankBalance(Integer.parseInt(bankTransferGetMatcher.group(2)));
            return;
        }

        Matcher lottoWinMatcher = PatternHandler.LOTTO_WIN.matcher(msg);
        if (lottoWinMatcher.find()) {
            this.unicacityAddon.services().file().data().addBankBalance(Integer.parseInt(lottoWinMatcher.group(1)));
            return;
        }

        Matcher cashGiveMatcher = PatternHandler.CASH_GIVE_PATTERN.matcher(msg);
        if (cashGiveMatcher.find()) {
            this.unicacityAddon.services().file().data().removeCashBalance(Integer.parseInt(cashGiveMatcher.group(2)));
            return;
        }

        Matcher cashTakeMatcher = PatternHandler.CASH_TAKE_PATTERN.matcher(msg);
        if (cashTakeMatcher.find()) {
            this.unicacityAddon.services().file().data().addCashBalance(Integer.parseInt(cashTakeMatcher.group(2)));
            return;
        }
        ...
        if (cashToFBankMatcher.find() && msg.contains(p.getName())) {
            this.unicacityAddon.services().file().data().removeCashBalance(Integer.parseInt(cashToFBankMatcher.group(1)));
            return;
        }
        ...
        if (cashFromFBankMatcher.find() && msg.contains(p.getName())) {
            this.unicacityAddon.services().file().data().addCashBalance(Integer.parseInt(cashFromFBankMatcher.group(1)));
            return;
        }

        Matcher cashToBankMatcher = PatternHandler.CASH_TO_BANK_PATTERN.matcher(msg);
        if (cashToBankMatcher.find()) {
            this.unicacityAddon.services().file().data().removeCashBalance(Integer.parseInt(cashToBankMatcher.group(1)));
            return;
        }
        ...
        if (cashFromBankMatcher.find()) {
            if (isGRBankCommand)
                return;
            this.unicacityAddon.services().file().data().addCashBalance(Integer.parseInt(cashFromBankMatcher.group(1)));
            return;
        }

        Matcher cashGetMatcher = PatternHandler.CASH_GET_PATTERN.matcher(msg);
        if (cashGetMatcher.find()) {
            this.unicacityAddon.services().file().data().addCashBalance(Integer.parseInt(cashGetMatcher.group(1)));
            return;
        }

        Matcher cashRemoveMatcher = PatternHandler.CASH_REMOVE_PATTERN.matcher(msg);
        if (cashRemoveMatcher.find()) {
            this.unicacityAddon.services().file().data().removeCashBalance(Integer.parseInt(cashRemoveMatcher.group(1)));
            return;
        }

        Matcher cashStatsMatcher = PatternHandler.CASH_STATS_PATTERN.matcher(msg);
        if (cashStatsMatcher.find()) {
            this.unicacityAddon.services().file().data().setCashBalance(Integer.parseInt(cashStatsMatcher.group(1)));
            return;
        }
