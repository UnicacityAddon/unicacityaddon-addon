...
public class TodoListCommand extends UnicacityCommand {
    ...
    public boolean execute(String[] arguments) {
        ...
        if (arguments.length == 0) {
            ...
            String todo = this.unicacityAddon.utilService().text().makeStringByArgs(arguments, " ").replace("done ", "");
            ...
            String todo = this.unicacityAddon.utilService().text().makeStringByArgs(arguments, " ").replace("delete ", "");
            ...
            String todo = this.unicacityAddon.utilService().text().makeStringByArgs(arguments, " ").replaceAll("(?i)edit " + arguments[1] + " ", "");
            TodolistEntry todolistEntry = new TodolistEntry(todo);
            this.unicacityAddon.fileService().data().getTodolist().set(index, todolistEntry);
            p.sendInfoMessage("Aufgabe editiert.");
        } else {
            String todo = this.unicacityAddon.utilService().text().makeStringByArgs(arguments, " ");
            TodolistEntry todolistEntry = new TodolistEntry(todo);
