...
public class GetGunCommand extends Command {
    ...
    public boolean execute(String prefix, String[] arguments) {
        ...
        if (arguments.length == 1) {
            armament = this.unicacityAddon.fileManager().data().getArmamentList().stream()
            ...
            boolean success = this.unicacityAddon.fileManager().data().removeArmamentPattern(arguments[1]);
            if (success) {
            ...
            if (weapon != null) {
                this.unicacityAddon.fileManager().data().addArmamentPattern(arguments[1], weapon, Integer.parseInt(arguments[3]));
    ...
    public List<String> complete(String[] arguments) {
        return TabCompletionBuilder.getBuilder(this.unicacityAddon, arguments)
                ...
                .addAtIndex(1, this.unicacityAddon.fileManager().data().getArmamentList().stream().map(Armament::getName).sorted().collect(Collectors.toList()))
                .addAtIndex(2, this.unicacityAddon.fileManager().data().getArmamentList().stream().map(Armament::getName).sorted().collect(Collectors.toList()))
