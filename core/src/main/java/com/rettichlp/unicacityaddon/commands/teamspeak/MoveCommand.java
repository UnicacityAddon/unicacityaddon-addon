 ...
 * @author RettichLP
 */
@UCCommand(prefix = "move", onlyOnUnicacity = false, usage = "[Spieler] [Channel]")
public class MoveCommand extends UnicacityCommand {
    ...
    public boolean execute(String[] arguments) {
        ...
        List<Client> clientsMoved = this.unicacityAddon.services().utilService().tsUtils().getClientsByName(Collections.singletonList(name));
        List<Client> clientsMoveTo = this.unicacityAddon.services().utilService().tsUtils().getClientsByName(Collections.singletonList(target));
