...
public class PatternHandler {
    ...
    public static final Pattern DRUG_GIVE_PATTERN = Pattern.compile("^\\[Deal] Du hast (?<issuer>(?:\\[UC])*(\\w+)) (?<amount>\\d+)g (?<drugType>.+) \\(Reinheit (?<drugPurity>\\d)\\) für (\\d+)\\$ angeboten\\.$");
    public static final Pattern DRUG_GET_PATTERN = Pattern.compile("^\\[Deal] (?<issuer>(?:\\[UC])*(\\w+)) bietet dir (?<amount>\\d+)g (?<drugType>.+) \\(Reinheit (?<drugPurity>\\d)\\) für (\\d+)\\$ an\\.$");
    ...
    public static final Pattern CASH_TO_FBANK_PATTERN = Pattern.compile("^\\[F-Bank] (?:\\[UC])*(?:\\w+) hat (\\d+)\\$ (|\\(-(\\d+)\\$\\) )in die Fraktionsbank eingezahlt\\.$");
    public static final Pattern CASH_FROM_FBANK_PATTERN = Pattern.compile("^\\[F-Bank] (?:\\[UC])*(?:\\w+) hat (\\d+)\\$ (|\\(\\+(\\d+)\\$\\) )aus der Fraktionsbank genommen\\.$");
    ...
    public static final Pattern DRUG_VAULT_INFOLSD_PATTERN = Pattern.compile("^ {2}» (LSD|Wundertüte|Wundertüten): (\\d+) Stück$"); // TODO adjust if bug is fixed on Unicacity
